            ...
            <tbody> <!-- Buka elemen tbody di sini -->
                ...
                foreach ($dataAkademik as $row) : ?>
                    <tr>
                        <td><?= $i++ ?></td>
                        <td>
                            <img src="/assets/images/akademik/<?= $row['foto'] ?>" alt="image" width="70px" />
                        </td>
                        <td><?= $row['nama'] ?></td>
                        <td><?= $row['username'] ?></td>
                        <td>
                            <div class="btn-group">
                                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#modalUpdate<?= $row['id_akademik']; ?>">
                                    <i class="mdi mdi-grease-pencil text-sm me-2"></i>
                                </button>
                                <a class="btn btn-danger" onclick="confirmDelete('<?= $row['id_akademik']; ?>')"><i class="mdi mdi-delete text-sm me-2"></i></a>
                            </div>
                        </td>
                    </tr>
                <?php endforeach; ?>
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modalCenterTitle">Input Data Akun <?= $page_title ?></h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <form action="/admin/userAkademikAdd" method="post" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-floating form-floating-outline mb-4">
                                    <input type="text" value="" class="form-control" name="nama" id="exampleFormControlInput1" placeholder="Masukan Nama akun akademik" autocomplete="off">
...
foreach ($dataAkademik as $row) : ?>
    <!-- Modal Update -->
    <div class="modal fade" id="modalUpdate<?= $row['id_akademik'] ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="modalCenterTitle">Update Data Administrator</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <form action="/admin/userAkademikUpdate/<?= $row['id_akademik'] ?>" method="post" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-floating form-floating-outline mb-4">
                                ...
                                <div class="col-md-3 mb-5">
                                    <div class="form-group text-center">
                                        <p>Foto Saat Ini</p>
                                        <img id="preview" src="/assets/images/akademik/<?= $row['foto'] ?>" alt="Preview" width="100px">
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "/admin/userAkademikDelete/" + id;
            } else {
